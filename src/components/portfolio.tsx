    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <article className="group flex flex-col rounded-xl border border-border bg-card overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-primary/40">
        <div className="relative aspect-[16/9] bg-secondary/60 overflow-hidden">
          {inProgress && (
            <span className="absolute top-2 left-2 z-10 inline-flex items-center gap-1 text-[11px] font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full bg-[#D4A017]/15 text-[#8a6508] border border-[#D4A017]/40 backdrop-blur-sm">
        ...
        <div className="flex flex-col flex-1 p-6">
          <div className="flex flex-wrap items-center gap-2 mb-3">
            {project.categories.map((c) => (
              <span
                key={c}
                className="text-[11px] font-semibold uppercase tracking-widest text-primary bg-primary/10 px-2 py-1 rounded-full"
              >
                {c}
              </span>
            ))}
            {project.badge && (
              project.badge !== "In Progress" && (
                <span className="inline-flex items-center gap-1 text-[11px] font-medium text-foreground/70 bg-secondary px-2 py-1 rounded-full">
                  {project.badge}
                </span>
              )
            )}
          ...
          <div className="mt-4 flex flex-wrap gap-1.5">
            {project.tools.split("·").map((t) => (
              <span
                key={t}
                className="text-[12px] px-2 py-0.5 rounded-full bg-secondary text-secondary-foreground border border-border"
              >
                {t.trim()}
              </span>
            ))}
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        ...
        >
          <p className="text-[12px] font-mono uppercase tracking-wider text-muted-foreground leading-tight">
            {c.period}
          </p>
          <p className="text-[12px] font-semibold leading-tight mt-0.5 text-foreground">
            {c.title}
          </p>
          {c.org && (
      ...
      <TooltipContent side={tipSide} className="max-w-xs text-left">
        ...
        {c.org && (
          <p className="text-[12px] text-foreground/70 mb-1">{c.org}</p>
        )}
        <p className="text-[12px] text-foreground/85 leading-snug">{c.notes}</p>
      </TooltipContent>
    </Tooltip>
  );
}
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        ...
        >
          ...
          <span className="text-[11px] font-medium text-[#7A5A0F] leading-none">
            {a.title}
          </span>
        </button>
      </TooltipTrigger>
      <TooltipContent side={tipSide} className="max-w-xs text-left">
        ...
        <p className="text-[12px] text-foreground/85 leading-snug">{a.notes}</p>
      </TooltipContent>
    </Tooltip>
  );
}
                ...
                <div className="grid grid-cols-[1fr_96px_1fr] items-stretch">
                  ...
                  <div className="relative flex flex-col items-center">
                    ...
                    <div className="absolute top-0 -translate-y-1/2 flex items-center gap-1.5">
                      ...
                      <span className="font-mono text-[12px] text-muted-foreground bg-background px-1.5 py-0.5 rounded">
                        {topYear}
                      </span>
                      <span className="h-px w-3 bg-foreground/50" />
                    </div>
